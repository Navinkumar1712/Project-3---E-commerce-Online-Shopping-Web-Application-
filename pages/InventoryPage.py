...
class InventoryPage(BasePage):
    ...
    def bike_light(self):
        """Method to fetch text of product"""
        bike_light_text  = self.retrieve_element_text("product_sauce_labs_bike_light_xpath",
                                                 self.product_sauce_labs_bike_light_xpath)
        return bike_light_text
    ...
    def click_on_logout_button(self):
        """Method to click on logout button reliably"""
        wait = WebDriverWait(self.driver, 10)
        logout_element = wait.until(EC.element_to_be_clickable((By.XPATH, self.logout_button_xpath)))
        self.driver.execute_script("arguments[0].scrollIntoView(true);", logout_element)
        ActionChains(self.driver).move_to_element(logout_element).click().perform()

    def click_on_reset_app_button(self):
        """Method to click on reset app state button with explicit wait"""
        self.click_on_burger_menu()      
        wait = WebDriverWait(self.driver, 10)
        reset_element = wait.until(EC.element_to_be_clickable((By.ID, "reset_sidebar_link")))
        reset_element.click()
        actions = ActionChains(self.driver)
        actions.move_to_element(reset_element).click().perform()
